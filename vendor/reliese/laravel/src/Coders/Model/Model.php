...
class Model
{
    ...
    protected function configure()
    {
        ...
        // Custom constants
        $this->withConstants($this->config('constants', []));
    ...
    {
        return empty($this->constants) ? []: $this->constants;
    }

    /**
     * @return array
     */
    public function getEnumsValues()
    {
        $enum = [];

        foreach ($this->getBlueprint()->columns() as $columnName => $column) {
            /* @var Fluent $column */
            foreach ($column->get('enum', []) as $value) {
                $constantName = $this->makeConstantName($columnName, $value);
                $enum[$constantName] = $value;
            }
        }
     ...
     * @return string
     */
    protected function makeConstantName($columnName, $value)
    {
        return strtoupper("{$columnName}_{$value}");
    }
     ...
     * @return $this
     */
    public function withValidator($name)
    {
        $this->validator = $name;
