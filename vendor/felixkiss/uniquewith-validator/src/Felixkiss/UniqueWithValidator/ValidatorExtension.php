...
class ValidatorExtension extends Validator
{
    ...
    public function __construct($translator, $data, $rules, $messages, array $customAttributes = array())
    {
        ...
        if(!isset($messages['exists_with']))
        {
            $messages['exists_with'] = $translator->get(
                'uniquewith-validator::validation.exists_with'
            );
        }
    ...
    public function validateUniqueWith($attribute, $value, $parameters)
    {
        ...
        list($connection, $table) = $this->parseTable($table);
    ...
    /**
     * Usage: exists_with: table, column1, column2, ...
     *
     * @param  string $attribute
     * @param  mixed  $value
     * @param  array $parameters
    ...
    {
        return !$this->validateUniqueWith($attribute, $value, $parameters);
    }

    public function replaceExistsWith($message, $attribute, $rule, $parameters)
    {
        return $this->replaceUniqueWith($message, $attribute, $rule, $parameters);
    }
     ...
     * @return string
     */
    protected function getColumnName($attribute, $parameters)
    {
        $column = isset($parameters[0]) ? $parameters[0] : $attribute;
        if ($column === $attribute || !preg_match("/^{(.*)}$/", $column)) {
            return $attribute;
        }
